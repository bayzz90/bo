 ...
 * Date Updated V.4 : 25-Agustus-2019
...
class Marlboro extends modules
{
	...
	private function login($email, $password)
	{
		...
		$login = $this->modules->curl("https://www.marlboro.id/auth/login", "email=".str_replace("@", "%40", $email)."&password=".$password."&ref_uri=%2F&decide_csrf=".$csrf."&param=&exception_redirect=false", true, false, true, $headers);
	...
	protected function view($idVidio)
	{
		...
		$view = $this->modules->curl("https://www.marlboro.id/article/video-play/".$idVidio, "decide_csrf=".$csrf."&log_id=false&duration=0.007&total_duration=0&fetch=1&g-recaptcha-response=", false, false, true, $headers);
		return $view;
	}
			...
			if($b%2 == 1)
			{
				...
				if(!empty($idVidio))
				{
					...
					if(strpos($view, '"message":"Success to store log play video."'))
					{
						...
						print PHP_EOL."Gagal Menonton ".$view;
					}	
				}else{
...
print PHP_EOL."Total Ada : ".count(explode(PHP_EOL, @file_get_contents($fileakun)))." Akun, Letsgo..";
